...
**孤儿恢复**:应用外改名导致 sidecar 失配时,扫描库:对每个无主 sidecar,用其 `system.contentHash` 与所有无 sidecar 的 `.md` 做哈希匹配,唯一命中即重新配对;多个命中(内容完全相同的笔记)或零命中则不自动配对,孤儿静置。纯哈希、不做模糊匹配——改名不改内容是绝对主流场景,哈希精确命中;启发式猜错的代价(把 A 的元数据贴到 B 上)比丢失元数据更糟。模糊匹配留作日后增强,schema 无需为它预留任何东西。
...
| 2 | 孤儿恢复 | 纯内容哈希,唯一命中才配对 | 模糊匹配:猜错代价高于丢失,V1 过度设计;不恢复:与 Phase 6 相悖,AI 层积累不可丢 |
